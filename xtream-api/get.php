...
if (!empty($playlist) && boolval($playlist['enabled']) && !empty($subscription)) {
    ...
    foreach ($groups as $group) {
        if ($group['group_type'] == 1) {
            ...
            $streams = $sql->sql_select_array_query("SELECT `id`, `user_id`, `playlist_id`, `group_id`, `stream_tvg_name`, `stream_tvg_id`, `stream_tvg_logo`, `stream_tvg_chno`, `stream_tvg_shift`, `stream_parent_code`, `stream_audio_track`, `stream_aspect_ratio`, `stream_radio`, `stream_order`, `movie_name`, `movie_year` FROM `movie` WHERE `user_id` = '{$playlist['user_id']}' AND `playlist_id` = '{$playlist['id']}' AND `group_id` = '{$group['id']}' AND `stream_is_hidden` = 0 AND `sync_is_removed` = 0 ORDER BY stream_order ASC");
        } elseif ($group['group_type'] == 3) {
            $streams = $sql->sql_select_array_query("SELECT `id`, `user_id`, `playlist_id`, `group_id`, `stream_tvg_name`, `stream_tvg_id`, `stream_tvg_logo`, `stream_tvg_chno`, `stream_tvg_shift`, `stream_parent_code`, `stream_audio_track`, `stream_aspect_ratio`, `stream_radio`, `stream_order`, `serie_name`, `serie_season`, `serie_episode` FROM `episodes` WHERE `user_id` = '{$playlist['user_id']}' AND playlist_id = '{$playlist['id']}' AND `group_id` = '{$group['id']}' AND `stream_is_hidden` = 0 AND `sync_is_removed` = 0 ORDER BY stream_order ASC");
